...
const PowerfulAIFeaturesSection = () => {
  return (
      ...
      <section className="w-full relative bg-[#032125] overflow-hidden h-auto min-h-[350px] md:h-[474px]">
        ...
        <div className="relative h-full flex items-center justify-start px-3 xs:px-4 sm:px-6 md:px-8 lg:px-12 xl:px-28 py-10 xs:py-12 sm:py-16 md:py-20 lg:py-24">
          <div className="w-full max-w-4xl space-y-4 xs:space-y-5 sm:space-y-6 md:space-y-8">
            {/* Badge */}
            <div className="w-16 xs:w-20 sm:w-24 md:w-28 h-6 xs:h-8 sm:h-9 px-1.5 xs:px-2 sm:px-3 bg-white/10 rounded-full outline outline-[1.26px] outline-offset-[-1.26px] outline-white/20 inline-flex justify-start items-center gap-1 xs:gap-1.5">
              <svg className="w-2.5 h-2.5 xs:w-3 xs:h-3 sm:w-3.5 sm:h-3.5 flex-shrink-0" width="15" height="15" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
              ...
              <span className="text-slate-300 text-[10px] xs:text-xs sm:text-[0.875rem] font-medium font-['Inter-400'] font-['Inter-sans'] leading-2.5 xs:leading-3 sm:leading-4 whitespace-nowrap">Features</span>
            </div>
            ...
            {/* Description */}
            <div className="w-full max-w-lg xs:max-w-xl sm:max-w-2xl md:max-w-4xl">
              <p className="text-white/70 text-sm xs:text-base sm:text-lg md:text-[1rem] font-normal font-['Inter-400'] font-['Inter-sans'] leading-relaxed xs:leading-relaxed sm:leading-relaxed md:leading-6">
                Everything you need to build intelligent AI agents that automate conversations and deliver exceptional customer experiences.
              </p>
            </div>
          </div>
        </div>
...
export default PowerfulAIFeaturesSection;
